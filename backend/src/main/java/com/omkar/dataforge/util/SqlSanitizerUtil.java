...
public class SqlSanitizerUtil {
    ...
    private static final Set<String> RESERVED_KEYWORDS = Set.of( "select", "table", "group", "order", "where", "from" );

    public static String sanitizeColumnName(String input) {

        String sanitized = input.trim().toLowerCase().replaceAll("[^a-z0-9_]", "_").replaceAll("_+", "_");
